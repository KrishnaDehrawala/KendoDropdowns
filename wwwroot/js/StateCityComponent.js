(function ($) {
    ...
    var StateCityWidget = Widget.extend({
        ...
        _create: function () {
            ...
            // Bind change event for state dropdown to update cities
            this._bindEvents();
        },
        ...
        _updateCities: function () {
            ...
            var cities = this._getCitiesByState(selectedStateId);
            // Update the cities dropdown data source
            citySelect.setDataSource([{
                text: "Select City", // Default label for the city dropdown
                value: "" // Value for "Select City" is empty
            }].concat(cities));
            if (selectedStateId) {
                citySelect.enable(true); // Enable the city dropdown once the data is loaded
            } else {
                citySelect.enable(false); // Disable city dropdown if no state is selected
            }
        },
